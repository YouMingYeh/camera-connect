...
export function Rotate() {
  ...
  const onRotate = async (angle: number) => {
    ...
    const {
      ...
      width: rotateWidth,
      height: rotateHeight,
    } = await ImageManipulator.manipulateAsync(originalImageData.uri, [{ rotate: angle }])
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.row, { paddingHorizontal: "20%" }]}>
        <IconButton iconID="rotate-left" text="Rotate -90" onPress={() => rotate("ccw")} />
        <IconButton iconID="rotate-right" text="Rotate +90" onPress={() => rotate("cw")} />
      </View>
      <View style={styles.row}>
        <IconButton iconID="close" text="Cancel" onPress={() => onClose()} />
        <Text style={styles.prompt}>Rotate</Text>
        <IconButton iconID="check" text="Done" onPress={() => setEditingMode("operation-select")} />
      </View>
    </View>
  )
}
