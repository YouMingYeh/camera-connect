...
const ImageCropOverlay = () => {
  ...
  const [cropSize, setCropSize] = (0, _recoil.useRecoilState)(_Store.cropSizeState)
  const [imageBounds] = (0, _recoil.useRecoilState)(_Store.imageBoundsState)
  const [accumulatedPan, setAccumluatedPan] = (0, _recoil.useRecoilState)(
    _Store.accumulatedPanState,
  ) // Editor context

  const { fixedAspectRatio, lockAspectRatio, minimumCropDimensions } = (0, React.useContext)(
    _index.EditorContext,
  )
  const [animatedCropSize] = React.useState({
    width: new _reactNative.Animated.Value(cropSize.width),
    height: new _reactNative.Animated.Value(cropSize.height),
  ...
  const panX = React.useRef(new _reactNative.Animated.Value(imageBounds.x))
  const panY = React.useRef(new _reactNative.Animated.Value(imageBounds.y))
  React.useEffect(() => {
  ...
  const onHandlerStateChange = ({ nativeEvent }) => {
    ...
    if (nativeEvent.state === _reactNativeGestureHandler.State.END) {
  ...
  const checkResizeBounds = ({ translationX, translationY }) => {
    ...
    const { x, y } = getTargetCropFrameBounds({
      translationX,
      translationY,
    })
        ...
        _reactNative.Animated.View,
        {
          style: [
            ...
            animatedCropSize,
            {
              transform: [
                {
                  translateX: _reactNative.Animated.add(panX.current, accumulatedPan.x),
                },
                {
                  translateY: _reactNative.Animated.add(panY.current, accumulatedPan.y),
                },
              ],
            },
          ],
